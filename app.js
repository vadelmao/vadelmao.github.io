...
// Referensi tombol HTML
const controlModeDisplay = document.getElementById("controlModeDisplay");
const toggleModeBtn = document.getElementById("toggleModeBtn");
const pumpOnBtn2 = document.getElementById("pumpOnBtn");
const pumpOffBtn2 = document.getElementById("pumpOffBtn");

let currentMode = "AUTO";

// --- Sinkronisasi mode dari Firebase ---
modeRef.on("value", snapshot => {
    const mode = snapshot.val() || "AUTO";
    currentMode = mode;

    // update display mode
    controlModeDisplay.innerText = mode;

    // update tombol mode
    toggleModeBtn.innerText = (mode === "AUTO") ? "Ubah ke MANUAL" : "Ubah ke AUTO";

    // update status enable/disable tombol pompa
    const isManual = (mode === "MANUAL");
    pumpOnBtn2.disabled = !isManual;
    pumpOffBtn2.disabled = !isManual;
});
...
toggleModeBtn.onclick = () => {
    ...
    modeRef.set(newMode);
    controlCommandRef.set("MODE_CHANGED:" + newMode);
};

// --- Tombol Pompa ON ---
pumpOnBtn2.onclick = () => {
    if (currentMode === "MANUAL") {
        controlCommandRef.set("PUMP_ON");
    }
};
...
pumpOffBtn2.onclick = () => {
    if (currentMode === "MANUAL") {
        controlCommandRef.set("PUMP_OFF");
    }
};
